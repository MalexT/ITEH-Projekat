...
if($id == "0"){
  ...
  $q = "SELECT * FROM prijava p join predmet pre on p.predmetID = pre.predmetID join kandidat k on p.brojKandidata=k.brojKandidata join rok r on p.rokID=r.rokID join instruktor ins on p.instruktorID = ins.instruktorID where k.brojKandidata='$id'";
}
